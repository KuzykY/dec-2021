...
for (let user of usersList) {
    ...
    for (let userKey in user) {
        ...
        if (typeof user[userKey]!== 'object'){
            ...
            keyDiv.innerText=`${userKey}`; ///addres
            for (let item in user[userKey]) {
                let itemDiv=document.createElement('div');
                if(typeof user[userKey][item]!=='object'){
                    itemDiv.innerText=`${item} : ${user[userKey][item]}`
                } else {
                    itemDiv.innerText=`${item}` ///geo
                    for (let itemKey in user[userKey][item]) {
                        let itemKeyDiv=document.createElement('div');
                        if(typeof user[userKey][item][itemKey]!=='object'){
                            itemKeyDiv.innerText=`${itemKey} : ${user[userKey][item][itemKey]}`
                        }else{
                            itemKeyDiv.innerText=`${itemKey}` ///company
                        }users.append(itemKeyDiv);

                    }
                }users.append(itemDiv)
